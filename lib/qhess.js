...
class Board {
    constructor() {
        this.board = [];
        this.emptyRow = [];
        for(i = 0; i < BOARD_SIZE; i++) this.emptyRow.push(new Space());
        for(i = 0; i < BOARD_SIZE; i++) this.board.push(this.emptyRow);
        let backRow = [Rook, Knight, Bishop, Queen, King, Bishop, Knight, Rook];
        for(let i = 0; i < BOARD_SIZE; i++) 
            this.board[i].addPiece(new backRow(0, i, 'BLACK'));
        for(let i = 0; i < BOARD_SIZE; i++) 
            this.board[i].addPiece(new Pawn(1, i, 'BLACK'));
        // add white pieces
        for(let i = 0; i < BOARD_SIZE; i++) 
            this.board[i].addPiece(new Pawn(6, i, 'WHITE'));
        for(let i = BOARD_SIZE-1; i >= 0; i++) 
            this.board[i].addPiece(new backRow(7, i, 'WHITE'));
    }
    hasPieceAt(x, y) {
...
class Bishop extends Piece {
    ...
    // Called on client
    getValidMoves(B) {
        moves = super.validPieceDiagonals(B, this.pos.x, this.pos.y);
        return moves;
    }
    // Called on server
    moveTo(x, y) {
...
class Rook extends Piece {
    ...
    // Called on client
    getValidMoves(B) {
        moves = super.validPieceHorizontal(B, this.pos.x, this.pos.y);
