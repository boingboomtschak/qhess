...
class Pawn extends Piece {
    ...
    moveTo(x, y, e) {
       ...
       if (x != this.pos.x && y != this.pos.x) {
            this.pos.x = x;
            this.pos.y = y;
       } else {
           this.pos.x = x;
           this.pos.y = y;
          // moves.filter(move => move[0] != this.pos.x)
       }
    }
}
...
function Probability(pos,E,T,P,A,D){
    return Math.pow(Wave(pos,E,T)*Barrier(pos,P,A,D), 2);
}

function getMove(listOfMoves, listOfBarriers, E){
    range = [];
    domain = [];
    interval = 0.05;
